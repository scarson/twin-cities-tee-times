...
> **Status — 2026-06-08:** ✅ **Implemented** on `feat/cps-profile-rotation` (PR to `dev`, Review-class — Sam merges). Delivered the recommended human-gated hybrid: an in-proxy versionless multi-vendor cascade (`chrome`/`safari`/`firefox`, axis 1, no redeploy) + a daily live-probe-gated GitHub Actions workflow that opens a `curl_cffi` bump PR when the pin ages out and a newer version both live-clears the real CPS challenge and cross-vendors for the Lambda (axis 2). Decision logic is a pure, exhaustively unit-tested function. Design: `docs/plans/2026-06-08-cps-profile-rotation-design.md`; plan: `docs/plans/2026-06-08-cps-profile-rotation-plan.md`; log: `docs/implementation-log.md`. **Owed:** the full `workflow_dispatch` run is deferred to post-merge-to-`main` (GitHub default-branch constraint).
