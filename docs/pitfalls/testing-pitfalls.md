...
- [ ] **A test helper must not swallow what the code under test throws:** The §1 rule applies to the test harness itself, not just production code. A `try { return await adapter.fetch(...) } catch { return [] }` helper converts every failure into the empty-result case, and any assertion downstream of it (`expect(Array.isArray(results)).toBe(true)`, or a `ctx.skip()` guarded on `results.length === 0`) then passes unconditionally. Catch only the specific, documented condition you mean to tolerate and rethrow everything else — `chronogolf.smoke.test.ts` is the reference: it swallows HTTP 403 alone, names why (Node undici's TLS fingerprint differs from the Workers runtime that succeeds in production), and returns an explicit `blocked` flag so tests can tell "blocked" from "no data." **🔥 Found 2026-07-19:** The TeeWire smoke suite reported green across 273 consecutive production failures — its helper caught every exception and its Level 1 assertion was `Array.isArray()`, which the `[]` fallback always satisfies. Production had never once succeeded for that platform.
...
- [ ] **Live tests must run on the runtime that production uses.** An outbound request's fate can depend on the runtime it originates from, not just the URL and headers — several booking CDNs admit or refuse a client by TLS fingerprint. A suite that runs somewhere production never runs is testing a transport no real request takes, and it fails or skips for reasons that tell you nothing about production health. Run live suites in the Workers runtime via `@cloudflare/vitest-pool-workers` (`vitest.smoke.config.mts`), and note that `process.env` is undefined there, matching the no-`process.env` rule (implementation-pitfalls CF-1) — pass values through bindings or inline them. **🔥 Found 2026-07-19:** Chronogolf's CDN returns 200 to the Workers runtime and 403 to Node's undici for the same IP, URL and headers. All three Chronogolf smoke levels had been self-skipping on that 403, leaving the largest platform in the catalog (26 courses) with no live coverage; moving the suite to the Workers runtime made them assert against real data.
